...
소스 12개 파일 3,815줄(테스트 3개와 `manifest.json`을 더하면 16개 4,693줄), 빌드 단계 없음, 의존성 없음, 개발자 한 명. 이 규모에서 설계 문서가
...
- 모든 서술은 **코드에서 확인된 것**만 담고, 필요한 곳에 `panel.js:417` 형태로 근거 위치를 답니다. 구현이 요구사항을 못 따라가는 자리는 감추지 않고
...
| 콘텐츠 스크립트 ↔ 같은 출처 API | `GET /api/trade/data/leagues`(`panel.js:910`), `POST /api/trade/search/<리그>`(`panel.js:976`). 둘 다 `credentials: 'include'` | 페이지와 같은 origin이라 **추가 권한이 필요 없다**(NFR-SEC-04). 로그인 쿠키는 브라우저가 알아서 실어 보내고 확장은 읽지 않는다 |
...
**나머지가 콘텐츠 스크립트인 이유.** 리그 목록과 검색 등록은 페이지와 **같은 출처**라 추가 권한 없이 나갑니다(`panel.js:910`, `:976`). 더 중요한 건
...
| 리그 목록 | `leagues` | origin별 24시간 | `panel.js:901-920` |
...
- **실패 (폼 못 읽음)**: 이름은 `suggestTitle`의 `<리그> <검색|대량거래> <검색ID>`로 남고 `summary`는 붙지 않습니다 (`:599`,
  FR-SUM-07).
...
- **실패 (재시작 후 만료)**: 10분이 지난 `pending`은 **무시하되 저장소에서 지우지 않습니다**(`:776`). 폼은 추천 이름으로 돌아가고, 저장하지 않은
  정규식은 되찾을 수 없습니다.
...
1. 목록 항목 클릭 → `openRecord`가 `markPanelNav(url)`로 `{url, at}`을 쓰고 `location.assign`(`panel.js:289`,
   `:394`). → 문서 교체 → 콘텐츠 스크립트 재시작.
2. `init`이 `takePanelNav(현재 URL)`을 부른다(`:1294`). **만료(60초 초과)이거나 주소가 일치하면 `remove(nav)`** 하고, 만료가
   아니면서 주소가 일치할 때만 값을 돌려준다 (`:407-408`). 주소가 맞지 않으면 표식은 그대로 남는다.
3. `recordHistory`가 `skipNavUrl === parsed.url`이면 **한 번만** 건너뛰고 표식을 비운다(`:422`). 그 뒤 이 페이지에서 새로 한
...
4. 각 모드의 `modTexts` — 원문과 **수치 구간을 첫 값으로 채운 표본** 둘 다에 대조한다(`trade-query.js:111-115`, `:162`). 모드 문구의
   ...
   않는다(FR-REGEX-09).
5. 패널이 `selected.clear()` 후 걸린 모드를 전부 넣어 **기존 선택을 대체**하고 프리셋 선택을 비운다(`panel.js:1228`, FR-REGEX-04).
...
- **반영 지점**: `storage.onChanged`가 다루는 것은 세 키뿐입니다 — `history`(목록 갱신), `userPresets`(드롭다운 갱신),
  `bookmarks`(목록 + 필요할 때만 폼)(`panel.js:696`, FR-BM-09·FR-PRESET-06).
...
- **경합**: 저장 경로가 모두 읽고-고쳐-쓰기입니다 — `getBookmarks() → 가공 → setBookmarks()`(`:633`), `getHistory() →
  setHistory()`(`:427`), 프리셋도 같습니다. 조건부 쓰기가 없으므로 두 탭이 거의 동시에 저장하면 **나중 쓰기가 앞 쓰기를 덮습니다**(lost update).
  겹치는 창이 밀리초 단위라 실제로 부딪히기는 어렵지만, 그것을 감수했다는 판단이 코드·주석 어디에도 남아 있지 않습니다.
- **`nav` 표식**: 만료됐거나 주소가 맞으면 지우고, 주소가 **맞지 않으면 남겨 둡니다**(`:407`) — 다른 탭이 집어삼켜 정작 이동한 탭이 기록을 남기는 일을
  막습니다(FR-HIST-08).
...
- **`summary`의 모양은 `{item, filters, ranges, statGroups}`입니다.** 쓰는 쪽은 `hasSummary(...)`일 때만 필드를
  붙이고(`panel.js:649`, `:664`), 보이는 쪽은 `formatSummary`가 그릴 때마다 줄을 만듭니다(`:303`). 툴팁 표기(`>= 16`, `[모두
...
- **없을 수 있는 필드는 옵셔널**입니다. `summary`는 폼을 못 읽었을 때, `regex`는 빌더로 만든 검색이 아닐 때, `updatedAt`은 한 번도 이름을 바꾸지
  않았을 때 붙지 않습니다. 읽는 쪽은 모두 폴백을 갖습니다 — 툴팁은 `formatSummary(...) || record.url`(`:303`), 정규식 버튼은
  `bookmark.regex`가 있을 때만 붙습니다(`:333`).
- **옛 북마크에 요약을 채워 넣는 경로**: 그 검색 페이지에서 이름을 **바꿔** 다시 저장하면 이름 변경 분기가 `currentSummary`를 함께 넣습니다(`:649`,
  FR-BM-10). 마이그레이션 단계는 없습니다. 이름을 바꾸지 않고서는 발동하지 않는 점이 요구사항과 어긋납니다(→ §9.3).
- **배열 키는 읽을 때 형태를 검사합니다** — `Array.isArray`가 아니면 `[]`로 봅니다(`:255`, `:383`, `:1032`). 옛 버전이 남긴 다른
  형태나 손상된 값에도 목록이 깨지지 않습니다.
- **TTL 키는 읽을 때 만료를 판정합니다.** `loadPending`은 만료된 값을 `null`로 돌려주되 **지우지는 않습니다**(`:776`) — 다음
  `savePending`이 덮거나 `clearPending`이 지울 때까지 남습니다.
...
| 확장 밖으로 데이터를 보내지 않음 | 나가는 요청은 셋뿐 — 영문 조회, 리그 목록, 검색 등록. 텔레메트리·통계 코드가 없음 | `background.js:38`, `panel.js:910`, `:976` |
| 추가 권한 없이 API를 씀 | 리그 목록·검색 등록은 페이지와 같은 origin으로 나가 호스트 권한이 필요 없음. `credentials: 'include'`이지만 브라우저가 그 사이트에 이미 가진 쿠키를 쓸 뿐 확장이 읽거나 저장하지 않음 | `panel.js:902-910`, `:976-981` |
...
| --- | --- | --- |
| 검색 등록 — 계정 5초당 3회, IP 60초당 15회 | 버튼에 `SEARCH_COOLDOWN_MS` 2.5초 최소 간격. 남은 시간은 초 단위로 안내하고 요청 자체를 보내지 않음 | `panel.js:724`, `:960-964` |
| 검색 등록 429 | `Retry-After` 값을 그대로 안내 문구에 넣음 | `panel.js:983-987` |
...
| 리그 목록 | origin별 `LEAGUE_CACHE_TTL_MS` 24시간 캐시. 실패하면 **그 로드 안에서는 재시도하지 않고** 박아 둔 목록으로 대체하고, 실패도 빈 목록도 캐시하지 않으므로(NFR-API-06) 다음 로드에서 다시 시도한다. 사용자가 **새로고침**을 누르면 캐시를 건너뛰고 한 번 더 받는다 | `panel.js:727`, `:901-920`, `:1026-1048` |
...
| 리그 목록 API 실패 | 드롭다운에 박아 둔 두 리그만 보이고, **박아 둔 목록을 쓰고 있다는 안내가 함께 뜸** (`panel.js:1370`, FR-BUILD-15) | 검색 등록. 다음 로드에서 다시 시도 |
| 새로고침 단추로 받다 실패 | "받지 못했습니다. 지금 목록을 그대로 둡니다." — **이미 받아 둔 목록을 폴백으로 갈아 끼우지 않음** (`panel.js:1034-1037`) | 드롭다운에 있던 리그 그대로 |
| 검색 등록 실패 | 상태 코드별 문구 — 429(대기 초), 401·403(로그인), 404(리그 이름), 400(조건), 그 밖의 HTTP 오류, 응답에 id 없음, 네트워크 오류 (`panel.js:945-956`) | 나머지 사이드바 전부 |
...
`panel.js:722`의 `FALLBACK_LEAGUES` 둘뿐이고, 평소에는 API에서 받은 목록을 씁니다.
...
| --- | --- | --- |
| `panel.js` 1,394줄이 사이드바 셸·북마크·기록·폼 채우기·빌더·모드 창·프리셋을 모두 담음 | 아니오. 파일 안이 절 주석으로 나뉘어 있고 절끼리 공유하는 상태는 `current`·`selected`·`pending` 정도임 | 한 절을 고칠 때 다른 절의 전역을 건드리게 되면. 쪼갠다면 경계는 그 주석선 그대로. 다만 콘텐츠 스크립트는 전역을 나눠 쓰므로 파일을 나눠도 스코프는 하나로 남고 `manifest.json` 순서 규칙이 늘어남 |
...
| 사이드바의 복사 두 곳에 클립보드 실패 처리가 없음 | 예 — 포커스가 없으면 아무 표시 없이 끝남 | 지금. `panel.js:341`(목록의 정규식 버튼)과 `:1330`(빌더 복사 버튼)가 `writeText`를 try/catch 없이 부름. EN 복사에는 폴백이 있음(`copy-en.js:68`) → §9.3 |
...
| FR-EN-09 (범위 밖) | 클립보드 API가 거부되면 `textarea` + `execCommand`로 한 번 더 시도한다 — **다만 이 요구사항은 EN 복사만 덮는다** | 사이드바의 복사 두 경로에는 폴백도 실패 처리도 없다. `writeText`를 try/catch 없이 부르므로 거부되면 아무 안내 없이 끝나거나 "복사됨"만 뜬다. 이를 덮는 요구사항이 없다 | `panel.js:340-344`(목록 정규식 버튼) · `:1328-1332`(빌더 복사 버튼) · 대비: `copy-en.js:68` |
...
| NFR-LEAGUE-05 | 지난 리그의 저장분이 남아 있어도 오동작하지 않고, **만료된 검색은 사용자가 알아볼 수 있어야 한다** | 앞은 지킨다(리그를 값으로만 저장해 깨지지 않는다). 뒤는 못 지킨다 — 만료 여부를 확장이 판단하지 않아 목록에는 리그 이름이 글자로만 붙고, 지난 리그 항목을 눌러도 "이미 저장된 검색입니다"라고만 말한다 | `panel.js:329`·`:453`(메타의 리그 글자) · `:590`(안내 문구) · ⚠ 확인 필요 → V3 |
...
| 성격 | 항목 | 사는 곳 | 리그가 바뀌면 |
| --- | --- | --- | --- |
| 변한다 (데이터) | 리그 이름·목록 | 런타임 `/api/trade/data/leagues` (`panel.js:910`), 캐시 `leagues` | 매 리그 반드시 바뀐다 |
| 변한다 (데이터) | 폴백 리그 이름 | `trade-query.js:44` `DEFAULT_LEAGUE`, `panel.js:722` `FALLBACK_LEAGUES` | 코드에 박혀 있어 저절로는 안 바뀐다 |
...
리그인지 판단하지 않습니다. 어기고 있는 곳은 `trade-query.js:44`의 `DEFAULT_LEAGUE`와 `panel.js:722`의
`FALLBACK_LEAGUES`입니다.

**원칙 2 — 리그 이름은 런타임에 받습니다. 하드코딩은 마지막 방어선일 뿐입니다.** 정상 경로는
`/api/trade/data/leagues`입니다(`panel.js:910`, FR-BUILD-06, NFR-LEAGUE-01). 박아 둔 이름은 API가 죽었을 때 확장이
통째로 멈추지 않게 하는 것이지 **평상시에 쓰이라고 있는 값이 아닙니다.** 그래서 `loadLeagues`는 목록만이 아니라 `{ leagues, fallback }`을 돌려주고
(`panel.js:901`), 폴백이 쓰였으면 부르는 쪽이 그 사실을 안내합니다(`:1370`, FR-BUILD-15). 폴백 값이 낡아 없는 리그로 POST가 나가면(`:976`) 그
실패는 상태 코드로 갈라져 리그 이름을 가리킵니다(`:945-956`, FR-BUILD-12) — 예전처럼 모든 실패를 로그인 문제로 몰지 않습니다. 박아 둔 이름 자체가 낡는 것은
...
| 1 | **새로고침 단추를 눌러 목록을 다시 받는다 (당일 필수).** 캐시는 24시간이라(`LEAGUE_CACHE_TTL_MS`, `panel.js:727`) 그냥 두면 새 리그가 최대 하루 동안 드롭다운에 안 뜬다. 빌더의 리그 줄에 있는 **새로고침**(`panel.js:1026`, FR-BUILD-13)이 캐시를 건너뛰고 받는다. 못 받으면 지금 목록을 그대로 두고 그 사실만 알리므로(`:1034-1037`) 눌러서 잃는 것은 없다 |
| 2 | **하드코딩된 리그 이름을 맞춘다.** 형식이 다른 두 곳을 **둘 다** 고친다 — `trade-query.js:44`의 `DEFAULT_LEAGUE`(문자열 리그 id)와 `panel.js:722`의 `FALLBACK_LEAGUES`(`{ id, text }` 배열, `Standard`는 남긴다). `id`는 API가 주는 영문 이름, `text`는 화면에 보이는 현지 이름이고 `/api/trade/data/leagues`에서 그대로 베낀다 |
...
| 리그 목록의 자동 추적 | 24시간이 지나면 저절로 새 목록을 받는다 (`panel.js:903-908`) |
| 리그 시작 당일의 캐시 | 24시간을 기다리지 않고 **새로고침** 단추가 캐시를 건너뛰고 받는다 (`panel.js:1026-1048`, FR-BUILD-13). 못 받았을 때 지금 목록을 폴백으로 갈아 끼우지 않는 것도 여기서 지킨다 (`:1034-1037`) |
| 저장해 둔 리그가 사라짐 | `fillLeagues`가 목록에 없는 저장분을 돌려주고(`panel.js:926-939`), 부르는 쪽이 어느 리그로 바뀌었는지 알린 뒤 저장분을 갱신한다 (`:1369`, `:1371`, FR-BUILD-14) |
| 폴백 목록을 쓴 사실 | `loadLeagues`가 `{ leagues, fallback }`을 돌려주므로 부르는 쪽이 박아 둔 목록임을 알린다 (`panel.js:901`, `:1370`, FR-BUILD-15). 진짜 목록으로 오해하지 않는다 |
| 빈 리그 목록 | 필터 결과가 비면 캐시하지 않고 폴백으로 넘긴다 (`panel.js:914`, NFR-API-06). 잘못된 상태가 24시간 고정되지 않는다 |
| 빌더 선택의 죽은 키 | `restoreSelection`이 `MAP_MODS`에 없는 키를 빼고 뺀 개수를 알린다 (`panel.js:1340-1348`, `:1368`, FR-BUILD-16). 개수 표시도 `selectedMods().length`라 실제 걸리는 모드 수와 갈리지 않는다 (`:872-881`) |
| 실패 원인의 구분 | `searchFailMessage`가 401·403(로그인) / 404(리그 이름) / 400(조건) / 그 밖으로 갈라 안내한다 (`panel.js:945-956`, FR-BUILD-12) |
| 프리셋의 없어진 모드 | 건너뛴 개수를 세어 사용자에게 알린다 (`panel.js:1245-1261`, FR-PRESET-05) |
...
| # | 견디지 못하는 것 | 증상 |
| --- | --- | --- |
| L4 | 낡은 리그 이름이 코드에 박혀 있음 | `FALLBACK_LEAGUES`(`panel.js:722`)와 `DEFAULT_LEAGUE`(`trade-query.js:44`)는 저절로 안 바뀐다. 리그 목록을 못 받은 날 없는 리그(`올플레임`)가 드롭다운에 뜨고 그 이름으로 POST가 나간다(`panel.js:968`, `:976`). **폴백을 썼다는 사실과 실패 원인은 이제 알린다** — 남은 것은 값 자체의 갱신이고, 형식이 다른 두 곳에 나뉘어 있어 한쪽만 고치기 쉽다 (§10.3 2단계) |
| L7 | 없어진 stat id로 나가는 검색 | 거래소가 거절하면 400으로 갈라져 "조건이 맞지 않는다"고 안내되지만(`panel.js:952-954`), **조용히 무시한다면** 검색은 성공하고 그 모드만 안 걸러진다. ⚠ 확인 필요 → V2 |
...
| L11 | 지난 리그 북마크·기록의 구분 | 목록에는 리그 이름이 글자로만 붙는다 (`panel.js:329`, `:453`). 만료 여부를 확장이 판단하지 않고 "이미 저장된 검색입니다"라고만 말한다 (`:590`, §9.3의 NFR-LEAGUE-05). ⚠ 확인 필요 → V3 |
...
| --- | --- | --- | --- |
| [design/detail-search.md](design/detail-search.md) (1편) | 검색 판독 · 북마크 · 기록 | `trade-url.js`, `search-summary.js`, `search-name.js` | `246-716` — 북마크 `246-361`, 검색 기록 `362-478`, 폼 채우기·감시·저장·`storage.onChanged` `479-716` |
| [design/detail-mapfilter.md](design/detail-mapfilter.md) (2편) | 인게임 정규식 엔진 · 8모드 빌더 · 프리셋 | `poe-regex.js`, `trade-query.js`, `map-mods.js`, `presets.js` | `719-1049` — 빌더 본체·리그·검색 실행. `1097-1373` — 프리셋·정규식 역선택·`initBuilder` |
| [design/detail-ui.md](design/detail-ui.md) (3편) | 사이드바 셸 · 모드 창 껍데기 · EN 복사 | `panel.css`, `copy-en.js`, `copy-en.css`, `background.js`, `manifest.json` | `1-245` — 상수·`PANEL_HTML`·호스트 심기·스타일·키 차단·여닫기. `1051-1095` — 모드 고르기 창 껍데기. `1375-1394` — `init` |
